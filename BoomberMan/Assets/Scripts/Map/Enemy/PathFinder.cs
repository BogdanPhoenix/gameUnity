...
namespace Map.Enemy
{
    public class PathFinder
    {
        ...
        {
            if (FreeNodes.Count == 0) return Vector2Int.zero;
        ...
        private IList<Vector2Int> GetPath(Vector2 target)
        {
            ...
            while (WaitingNodes.Count > 0)
            {
                ...
                if (nodeToCheck!.Position == targetPosition) return CalculatePathFromNode(nodeToCheck).ToList();
        ...
        private IEnumerable<Vector2Int> CalculatePathFromNode(Node node)
        {
            ...
            {
                if (CheckWalkable(currentNode)) path.Add(currentNode.Position);
