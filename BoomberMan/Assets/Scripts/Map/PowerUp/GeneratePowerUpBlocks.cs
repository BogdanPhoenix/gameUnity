...
namespace Map.PowerUp
{
    public class GeneratePowerUpBlocks : GenerateObject
    {
        ...
        private readonly IList<PowerUpElement> PowerUoElements;
        
        public GeneratePowerUpBlocks(IList<GameObject> elements, GameObject bomberMan)
        {
            Elements = elements;
            BomberMan = bomberMan;
            PowerUoElements = new List<PowerUpElement>();

            foreach (var element in elements)
            {
                PowerUoElements.Add(element.GetComponent<PowerUpElement>());
            }
        }
        ...
        {
            var totalProbability = PowerUoElements.Aggregate<PowerUpElement, float>(0, (current, element) => current + element.Weight);
            var randomValue = Random.Range(0f, totalProbability);
            var cumulativeProbability = 0f;
            
            for (var i = 0; i < PowerUoElements.Count; i++)
            {
                cumulativeProbability += PowerUoElements[i].Weight;
